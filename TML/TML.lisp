...
(defmacro handle-base (this-expr input-expr)
    (setl res-sym (gensym))
    (setl interaction-sym (gensym))
    `(progn 
        (if (&& (,not (,eq (index ,this-expr (quote interaction-stack)) null)) (> (len (index ,this-expr (quote interaction-stack))) 0))
            (setl ,interaction-sym  (,back (index ,this-expr (quote interaction-stack))))
            (setl ,res-sym (,handle-input (,index ,interaction-sym 'element) ,input-expr) )
            (if (not ,res-sym)
                (set-focus (,index ,interaction-sym 'element) false)
                (,pop ,this-expr)
                (return (,not (,index ,interaction-sym  'replaces-input)))
            )
            (return true)
         else if (not (,eq (index ,this-expr (quote input)) null))
...
(defclass TMLElement (ElementBase)
    ...
    (clear-children ()
        (clear-children :stacker_ this)
    )
    (add-child (child)
        (append :children this child)
        (set-children this :children this)
    )
    (constructor (children)
        (set-parent-info :stacker_ this :parent this)
    ...
    (prefered-dims (dimensions)
        (prefered-dims :stacker_ this dimensions)
    )
...
(defclass interaction-element ()
    ...
    (constructor (elem callback replaces-input)
        (setl :element this elem)
        (setl :pop-callback this callback)
        (setl :replaces-input this replaces-input)
    )
)
...
(defmethod push ((this TMLElement) sub-element callback replaces-input)
    (if (not (eq current-interaction-stack null))
        (append current-interaction-stack (interaction-element sub-element callback replaces-input))
     else
        (set :interaction-stack this (list))
        (append :interaction-stack this (interaction-element sub-element callback replaces-input))
    )
)

(defmethod pop ((this TMLElement))
    (if (&& (not (eq :interaction-stack this null)) (> (len :interaction-stack this) 0))
        (setl top (pop :interaction-stack this))
